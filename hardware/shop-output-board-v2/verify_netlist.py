...
import re
import sys
from generate_schematic import tokenize, parse, N_CHANNELS, CHANNEL_GP, GP_TO_PICO_PIN
...
def main():
    ...
    for n in range(1, N_CHANNELS + 1):
        ...
        # Indicator anode (pin 2) faces R4/OUT; cathode (pin 1) goes to ground. Asserting
        # this explicitly is the point: v1 had it backwards and this check ratified it.
        expect(f"IND{n}", {f"R{n}04.2", f"LED{n}.2"})
        gp = CHANNEL_GP[n - 1]
        expect(f"GP{gp}", {f"R{n}05.1", f"J4.{GP_TO_PICO_PIN[gp]}"})

        # v2 has a single 6-way output terminal: 4 outputs plus two grounds.
        expect(f"OUT{n}", {f"Q{n}.{FET_D}", f"R{n}03.1", f"R{n}04.1", f"J2.{n}"})
    ...
    # --- Terminals ----------------------------------------------------------------------
    expect("ISO_GND", {"J2.5", "J2.6"}, exact=False)
