...
what hardware/shop-output-board-v2/netlist.md specifies, and leaves nothing to go subtly wrong
...
# v2 is a four-channel board. Everything downstream derives its channel count from the
# length of this list, so shortening it is the whole change — generate_pcb.py, both
# verifiers and the diagram generators all read it rather than counting for themselves.
CHANNEL_GP = [2, 3, 4, 5]
N_CHANNELS = len(CHANNEL_GP)
...
MPN = {
    ...
    # Half the bulk of v1's 100uF, because half the channels means half the coil inrush.
    # Body drops from 8x10.5 mm to 6.3x7.7 mm, which is most of what shrinks the input row.
    "47uF/50V":          ("EEE-FK1H470XP",     "Panasonic"),
...
FOOTPRINTS = {
    ...
    "Device:C_Polarized": "Capacitor_SMD:CP_Elec_6.3x7.7",
...
def uid(seed=None):
    ...
    h = hashlib.sha1(f"shop-output-board-v2::{seed}".encode()).hexdigest()
...
def main():
    ...
    for n in range(1, N_CHANNELS + 1):
        ...
        ind = f"IND{n}"   # R4 -> indicator LED anode
        leda = f"LEDA{n}"
        ...
        net(p, "2", ind, f"R{n}04", "down")

        # KiCad's Device:LED is pin 1 = K, pin 2 = A. The node coming down from R4 is at
        # OUT potential, so it must land on the ANODE. v1 had these swapped, which
        # reverse-biases the indicator and it simply never lights. netlist.md always
        # specified it correctly; the generator did not.
        p = place(f"LED{n}", "Device:LED", "GRN", cx + 25, TOP + 145)
        net(p, "2", ind, f"LED{n}", "up")      # pin 2 = A, to R4 / OUT
    ...
    p = place("C1", "Device:C_Polarized", "47uF/50V", ix + 210, iy)
    net(p, "1", "V+", "C1", "up")
...
'''
    ...
    sch_path = os.path.join(out_dir, "shop-output-board-v2.kicad_sch")
    with open(sch_path, "w", encoding="utf-8") as fh:
        fh.write(sheet)

    pro_path = os.path.join(out_dir, "shop-output-board-v2.kicad_pro")
    if not os.path.exists(pro_path):
        with open(pro_path, "w", encoding="utf-8") as fh:
            fh.write('{\n  "board": {},\n  "meta": {"filename": "shop-output-board-v2.kicad_pro", "version": 3},\n  "sheets": [["%s", "Root"]]\n}\n' % SHEET_UUID)
